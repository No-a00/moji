...
const MessageItem = ({message,index,messages,selectedconvo,lastMessageStatus}:MessageItemProp) => {
  ...
  const participant = selectedconvo.participants.find((p:Participant)=>p._id.toString()===message.senderId.toString())
    ...
    <div className={cn("flex gap-2 message-bounce",message.isOwn?"justify-end":"justify-start")}>
        ...
        {/* tin nhắn */}
        <div className={cn("mxa-w-xs lg:max-w-md space-y-1 flex flex-col",message.isOwn?"items-end":"items-start")}>
        ...
        {message.isOwn&&message._id===selectedconvo.lastMessage?._id&&(
            <Badge 
                variant="outline"
                className={cn("text-xs px-1.5 py-0.5 border-0 ",lastMessageStatus==="seen"?"bg-primary/20 text-primary":"bg-muted text-muted-foreground")}        
                >
                {lastMessageStatus}
            </Badge>
        )}
